...
const WhyJoinUsForm = ({ initialData, companyId }: WhyJoinUsFormProps) => {
  ...
  const handlePromptGeneration = async () => {
    try {
      setisPrompting(true);
      const customPrompt = `Create a compelling "Why Join us" content piece for ${roleName}. highlight the unique opportunities, benefits , and experiences that ${roleName} offers to its users. Emphasize the platform's value proposition, such as  personalized recommendations, exclusive content, comunity features, and career opportunities.Tailor the content to attract potential users and illustrate why ${roleName} stands out among other companies`;
  ...
  return (
    <div className="mt-6 border bg-neutral-100 rounded-md p-4">
      ...
      {!isEditing && <div className={cn("text-sm mt-2", !initialData.whyJoinUS && "text-neutral-500 italic")}>{!initialData.whyJoinUS ? "No Details" : <Preview value={initialData.whyJoinUS} />}</div>}
